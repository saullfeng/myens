...
contract AdminInEth is Ownable, ILayerZeroReceiver {
    ...
    function _LzReceive(
        ...
        // (address owner, uint256 label, uint256 expiration,string memory name) = abi.decode(
        //     _payload,
        //     (address, uint256, uint256, string)
        // );

        (address owner, uint256 label, uint256 expiration) = abi.decode(
            _payload,
        ...
        if (base.isExist(label)){
           IDeposit(deposit).withdraw(label, owner);
        } else {
            base.register(label, owner, expiration);
        }
