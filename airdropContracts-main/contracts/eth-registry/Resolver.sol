...
contract Resolver is Ownable {
    ...
    NodeController public immutable nodecontroller;
    mapping(bytes32 => uint64) public recordVersions;
    mapping(bytes32 => mapping(uint => bytes)) _addresses;
    mapping(uint64 => mapping(bytes32 => bytes)) versionable_hashes;
    mapping(uint64 => mapping(bytes32 => mapping(string => string))) versionable_texts;
    mapping(uint64 => mapping(bytes32 => string)) public versionable_names;
    ...
    constructor(NodeController _nodecontroller) {
    ...
    modifier authorised(bytes32 node) {
        ...
        require(
            owner == msg.sender ||
                nodecontroller.isApprovedForAll(owner, msg.sender)
        );
        _;
    }
    ...
    function _addr(bytes32 node, uint coinType)
        public
        view
