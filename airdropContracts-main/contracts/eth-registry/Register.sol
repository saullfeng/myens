...
contract Register is IRegister, ERC721, Ownable {
    ...
    modifier isCodeValidMerkleProof(
        bytes32[] calldata _merkleProof,
        bytes32 _root,
        ...
        require(
            MerkleProof.verify(
                _merkleProof,
                _root,
                keccak256(abi.encodePacked(code, amount))
            ),
            "code or  amount does not exist in list"
        );
        _;
    }

    modifier isNumber(uint256 _number) {
        require(
            _number < Number,
    ...
    {
        return _batchregister(id, owner, duration);
    }
    ...
    {
        return _register(id, owner, duration, number);
    }
    ...
    {
        return _register(_merkleProof, _root, id, owner, duration);
    }
    ...
    function _register(
        ...
        internal
        isValidMerkleProof(_merkleProof, _root, owner)
        returns (
            // onlyController
            // live
            //string memory name
    ...
    {
        return _codeRegister(_merkleProof, _root, id, owner, duration, code);
    }

    function _codeRegister(
        bytes32[] calldata _merkleProof,
        bytes32 _root,
        uint256 id,
        address owner,
        ...
        internal
        isCodeValidMerkleProof(_merkleProof, _root, code, duration)
        returns (uint256)
    {
        require(available(id), "id is fail");
        require(
